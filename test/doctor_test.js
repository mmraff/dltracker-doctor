      ...
      it('should be same as data passed back by tracker.audit()', function(done) {
        ...
        function nextBadJsonTest(i) {
          ...
          ut.copyFile(src, tgt, function(err) {
            if (err) return done(err)
            createTrackerAsync(tempDir1)
            .then(tracker => promisify(tracker.audit)())
            .then(auditData => {
...
/*
  The problem with that is that the Errors generated by two different runs of
  audit() have different stack traces, so they're never going to be equal even
  when they have the same message and the same code.
  It will work well to use deep equal on the data property, but as for the
  errors, it only makes sense to compare the codes.
*/
    ...
    describe('saveState()', function() {
      ...
      it("should result in no remaining problems when called after all items marked resolved", function(done) {
        createDoctor(tempDir1).then(doctor => {
          const allErrData = doctor.data()
          // Of course, this is *not* the recommended approach for users to deal with problems:
